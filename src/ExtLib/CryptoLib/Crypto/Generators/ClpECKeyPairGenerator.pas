...
type
  ...
  public
    ...
    function GeneratePhraseKeyPair(const Phrase: String): IAsymmetricCipherKeyPair;
...
function TECKeyPairGenerator.GeneratePhraseKeyPair(
  const Phrase: String): IAsymmetricCipherKeyPair;
var
  n, d: TBigInteger;
  minWeight: Int32;
  q: IECPoint;
...
begin
  ...
  while (true) do
  begin
    b := ctxHash.GetHashBytes(s,SHA256);
    d := TBigInteger.Create(b);
    if ((d.CompareTo(TBigInteger.One) < 0) or (d.CompareTo(n) >= 0)) then
    begin
      s := d.ToString;
      continue;
