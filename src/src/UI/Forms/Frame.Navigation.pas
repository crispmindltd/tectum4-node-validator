...
type
  TNavigationFrame = class(TFrame)
    ...
    procedure PageEditKeyDown(Sender: TObject; var Key: Word;
...
begin
  ...
  Width := GetContentRect(PagesPanelLayout).Width + PrevPageLayout.Width + NextPageLayout.Width + PageEdit.Width + 28;
...
procedure TNavigationFrame.PageEditKeyDown(Sender: TObject; var Key: Word;
  var KeyChar: WideChar; Shift: TShiftState);
begin
  var val: Integer;
  if (ord(Key) = 13) and TryStrToInt(PageEdit.Text, val) then
  begin
    PageNum := Min(PagesCount, Max(val, 1));
    PageEdit.Text := PageNum.ToString;
    DoChange;
  end;
end;
