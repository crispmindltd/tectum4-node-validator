...
function BytesToHex(const Bytes: TBytes; isForLog:Boolean = False): string;
...
begin
  Result := '';
  if isForLog and (Length(Bytes) > 400) then
    Result := BytesToHex(Copy(Bytes, 0, 200)) + '..' + BytesToHex(Copy(Bytes, Length(Bytes) - 200))
  else
