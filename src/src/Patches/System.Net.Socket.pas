...
{$ENDIF}
  ...
  protected
    ...
    /// </summary>
    function WaitForData(ATimeout: Int64 = INFINITE): TWaitResult;
    /// <summary>
  ...
  public
    ...
    ///    0 and -1 means infinite timeout. Default value is 0.
    /// </summary>
    ...
    /// <summary>
    ///    Property specifies connect timeout in msecs.
    ///    0 means default system timeout. Default value is 0.
    /// </summary>
...
function SetKeepAlive(socket: TSocketHandle; KeepAliveTime: Cardinal; KeepAliveInterval: Cardinal): Boolean;
//(SetKeepAlive(FClient.Handle,2000,1000)
...
begin
  ...
  if Integer(ClientHandle) < 0 then
    Result:= nil
  else
...
begin
  Result := TSocketReceiveAsyncResult.Create(Self,
    function: TBytes
    var
      Buffer: array[0..1024 * 5] of Byte;
    begin
      Count := recv(FSocket, Buffer, SizeOf(Buffer), 0);
      if Count > 0 then
        Result := BytesOf(@Buffer, Count)
      else // Ignore all socket errors
        Result := nil;
...
const
  SO_REUSEPORT =  15 platform;
  V_ON: Integer = 1;

procedure TSocket.DoListen(QueueSize: Integer);
begin
{$IFDEF LINUX64}
  setsocketoption(FSocket, SOL_SOCKET, SO_REUSEPORT, V_ON, SizeOf(V_ON));
{$ENDIF}
  DoBind;
...
{$IFNDEF ANDROID}
  ...
  begin
    // nonblocking mode
    Mode := 1;
    CheckSocketResult(ioctlsocket(Handle, Integer(FIONBIO), Mode), 'ioctlsocket');

    Res := socketconnect(Handle, sockaddr(FAddr), SizeOf(FAddr));
    // https://stackoverflow.com/questions/6202454/operation-now-in-progress-error-on-connect-function-error
    if (Res < 0) and (GetLastError <> {$IFDEF MSWINDOWS} WSAEINPROGRESS {$ELSE} EINPROGRESS {$ENDIF}) then
      CheckSocketResult(Res, 'connect');
    ...
    FDR := TFDSet.Create(Self);
    if Select(FDR, FDR, FDR, ConnectTimeout * 1000) = TWaitResult.wrTimeout then
      raise ESocketError.CreateResFmt(@sSocketTimeout, ['select']);
  end
  else
{$ENDIF}
    CheckSocketResult(socketconnect(Handle, sockaddr(FAddr), SizeOf(FAddr)), 'connect');
  FState := [TSocketState.Connected, TSocketState.Client];
end;
...
begin
  ...
  Result:=
    (setsocketoption(socket,SOL_SOCKET,SO_KEEPALIVE,KeepAliveOn,SizeOf(KeepAliveOn))=0) and
    (WSAIoctl(socket,SIO_KEEPALIVE_VALS,@ka,SizeOf(ka),nil,0,Bytes,nil,nil)=0);
...
const
  TCP_KEEPIDLE = 4 platform;
  TCP_KEEPINTVL = 5 platform;
  TCP_KEEPALIVE = $10 platform;

function SetKeepAlive(socket: TSocketHandle; KeepAliveTime: Cardinal; KeepAliveInterval: Cardinal): Boolean;
const KeepAliveOn: Integer=1;
begin

  KeepAliveTime:=KeepAliveTime div 1000;
  KeepAliveInterval:=KeepAliveInterval div 1000;

  {$IFDEF MACOS}

  Result:=
    (setsocketoption(socket,SOL_SOCKET,SO_KEEPALIVE,KeepAliveOn,SizeOf(KeepAliveOn))=0) and
    (setsocketoption(socket,IPPROTO_TCP,TCP_KEEPALIVE,KeepAliveTime,SizeOf(KeepAliveTime))=0);

  {$ELSE}

  Result:=
    (setsocketoption(socket,SOL_SOCKET,SO_KEEPALIVE,KeepAliveOn,SizeOf(KeepAliveOn))=0) and
    (setsocketoption(socket,IPPROTO_TCP,TCP_KEEPIDLE,KeepAliveTime,SizeOf(KeepAliveTime))=0) and
    (setsocketoption(socket,IPPROTO_TCP,TCP_KEEPINTVL,KeepAliveInterval,SizeOf(KeepAliveInterval))=0);
...
function TSocket.WaitForData(ATimeout: Int64): TWaitResult;
var
  ReadFds: TFDSet;
  LTimeout: Int64;
begin
  ReadFds := TFDSet.Create(Self);
  if (ATimeout >= 0) and (ATimeout <> INFINITE) then
