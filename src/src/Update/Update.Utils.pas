...
procedure CheckInstallPackage(const PackageApp: string);
...
procedure CheckUnzipAppFile(const ZipFileName,AppFileName: string);
begin
  ...
  Require(ZipFile.IndexOf(AppFileName)<>-1, 'not found appfile in zip');
...
begin
  ...
  var AppFileIndex := ZipFile.IndexOf(ThisFileName);

  Result := AppFileIndex<>-1;

  if Result then
    ZipFile.Extract(AppFileIndex,DestPath);
...
begin
  if FileIsZip(PackageApp) then
    CheckUnzipAppFile(PackageApp,ExtractFileName(TFolder.GetAppPath));
end;
